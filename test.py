...
HOLD_THRESH   = 0.003
FEE_PCT       = 0.001   # 0.1% poplatek na každý obchod (realistický broker)

MARKET_TICKERS = {
    "sp500": "^GSPC", "nasdaq": "^IXIC", "vix": "^VIX",
    "gold": "GC=F", "oil": "CL=F", "bonds": "^TNX",
    "dax": "^GDAXI", "dxy": "DX-Y.NYB",
}
SECTOR_ETFS = {
    "tech_etf": "XLK", "finance_etf": "XLF", "health_etf": "XLV",
    "energy_etf": "XLE", "consumer_etf": "XLY",
}
...
def dl_hourly(ticker, period="60d"):
    try:
...
def build_features_at(df_h_full, df_d_full, market_h, market_d,
    ...
    if not df_d_full.empty:
        ...
        if not d.empty:
            ...
            orig_idx = h_feat.index.copy()
            date_idx = pd.to_datetime(h_feat.index.date)
            d_feat.index = pd.to_datetime(d_feat.index.date)
            overlap = [c for c in d_feat.columns if c in h_feat.columns]
            d_feat = d_feat.rename(columns={c: f"{c}__d_main" for c in overlap})
            h_feat.index = date_idx
            h_feat = h_feat.join(d_feat, how="left")
            h_feat.index = orig_idx
    ...
    for name, mdf in market_h.items():
        ...
        if mdf_cut.empty:
            continue
        overlap = [c for c in mdf_cut.columns if c in h_feat.columns]
        mdf_cut = mdf_cut.rename(columns={c: f"{c}__{name}" for c in overlap})
        mdf_cut = mdf_cut.shift(1)  # lag → kein look-ahead
        h_feat = h_feat.join(mdf_cut, how="left")
    ...
    for name, mdf in market_d.items():
        ...
        orig_idx = h_feat.index.copy()
        date_idx = pd.to_datetime(h_feat.index.date)
        mdf_cut.index = pd.to_datetime(mdf_cut.index.date)
        overlap = [c for c in mdf_cut.columns if c in h_feat.columns]
        mdf_cut = mdf_cut.rename(columns={c: f"{c}__{name}" for c in overlap})
        h_feat.index = date_idx
        h_feat = h_feat.join(mdf_cut, how="left")
...
def run_strict_backtest(model_dir, tickers, n_snapshots, horizon_h):
    print(f"\n{'='*65}")
    print(f"  Mičov — STRICT BACKTEST (ChatGPT audit check)")
    print(f"  Snímků: {n_snapshots}  Horizont: {horizon_h}h  Tickery: {len(tickers)}")
    print(f"{'='*65}\n")
    ...
    ref_df = dl_hourly(tickers[0], period="60d")
    if ref_df.empty:
    ...
    # Ověření: jsou snímky skutečně po tréninkových datech?
    pairs = []
    last_snap_used = trading_idx[-1] + timedelta(days=1)
    for snap_t in reversed(trading_idx):
        if (last_snap_used - snap_t).total_seconds() / 3600 < horizon_h:
            continue
        eval_target = snap_t + timedelta(hours=horizon_h)
        candidates = [t for t in trading_idx
                      if t >= eval_target and
                      (t - eval_target).total_seconds() / 3600 <= horizon_h + 2]
        if not candidates:
            continue
        eval_t = min(candidates, key=lambda t: abs((t - eval_target).total_seconds()))
        if eval_t <= snap_t:
            continue
        pairs.append((snap_t, eval_t))
        last_snap_used = snap_t
    ...
    for ticker in tickers:
        try:
            df_h = dl_hourly(ticker, period="60d")
